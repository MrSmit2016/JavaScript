...
const gallery = [
  ...
  const galleryItems = gallery.reduce((acc,el)=> acc + `<li class="gallery__item"><img class="gallery__image" data-source=${el.original} src=${el.preview} alt=${el.description} /></li>`, "");

  galleryList.innerHTML = galleryItems;

  function eventHandler(e){
    if(e.target.nodeName === "IMG" && e.target.classList.contains("gallery__image")){
        lightbox.children[1].firstElementChild.setAttribute("src", e.target.dataset.source);
